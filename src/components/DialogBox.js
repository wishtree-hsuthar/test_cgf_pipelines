...
const DialogBox = ({
  ...
  let dialogTitle =""
  typeof title?.props?.children === "object" ? title?.props?.children?.map((value) =>  dialogTitle += value) : dialogTitle = title?.props?.children
        ...
        <Box sx={style} className="popup-box">
          ...
          <div id="transition-modal-description" className="popup-body">
            <div className="popup-content-blk text-center">
              {info1}
              {isModalForm ? (
                <form onSubmit={onSecondaryButtonClickHandler}>{info2}</form>
              ) : (
                info2
              )}
              {/* {info2 && <p>{info2}</p>} */}
