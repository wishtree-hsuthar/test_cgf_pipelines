...
export const getCategories = async () => {
  try {
    ...
    Logger.info(`Member module util- getCategories handler`);
    return updateLookUp(response?.data);
  } catch (error) {
    Logger.info(
      `Member module util- getCategories handler ${error?.response?.data?.message}`
    );
  }
};
...
export const getCGFOffices = async () => {
  try {
    ...
    Logger.info(`Member module util- getCGFOffices handler`);

    return response?.data;
  } catch (error) {
    Logger.info(
      `Member module util- getCGFOffices handler catch error${error?.response?.data?.message}`
    );
  }
};
