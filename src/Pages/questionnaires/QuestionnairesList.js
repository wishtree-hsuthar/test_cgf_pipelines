...
function QuestionnairesList() {
  ...
  const [searchTimeoutQuestionnaire, setSearchTimeoutQuestionnaire] =
    useState(null);
  //state to hold wheather to make api call or not
  const [makeApiCallQuestionnaire, setMakeApiCallQuestionnaire] =
    useState(true);
  ...
  let moduleAccesForMember = QuestionnairePrivilgeArray.filter(
    (data) => data?.moduleId?.name === "Questionnaire"
  ).map((data) => ({
    questionnaire: {
      list: data?.list,
      view: data?.view,
                ...
                <div className="member-tab-wrapper">
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      <Tab label="Drafted" {...a11yPropsQuestionnaireList(1)} />
                    </Tabs>
                  </Box>
                </div>
              </div>
          ...
          <div className="member-info-wrapper table-content-wrap">
            <TabPanel value={questionnaireValue} index={1}>
              <DraftedQuestionnaires
                pageDraftedQuestionnaire={pageDraftedQuestionnaire}
                setPageDraftedQuestionnaire={setPageDraftedQuestionnaire}
            ...
            <TabPanel value={questionnaireValue} index={0}>
              <PublishedQuestionnaires
                pagePublishedQuestionnaire={pagePublishedQuestionnaire}
                setPagePublishedQuestionnaire={setPagePublishedQuestionnaire}
                makeApiCall={makeApiCallQuestionnaire}
                setMakeApiCall={setMakeApiCallQuestionnaire}
                search={searchQuestionnaire}
                setSearch={setSearchQuestionnaire}
