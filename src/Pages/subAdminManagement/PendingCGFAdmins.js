...
function PendingCGFAdmins({
  ...
  const updatePendingRecords = (data) => {
    ...
    console.log("data in updaterecords method in pending method", staleData);
  ...
  const getSubAdminPending = async (
    ...
    try {
      ...
      setTotalRecordsForPendingTab(parseInt(response.headers["x-total-count"]));
      console.log(
      ...
      console.log("Error from getSubAdmin pending tab table-------", error);
      isMounted &&
        setToasterDetails(
          {
            titleMessage: "Error",
