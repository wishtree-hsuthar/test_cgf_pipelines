...
function FillAssessment() {
  ...
  console.log(
    "first user ",
    userAuth._id + "  second user  " + assessment?.assignedOperationMember?._id
  );
  ...
  useEffect(() => {
    ...
    // );
    setViewMode(
      userAuth._id === assessment?.assignedOperationMember?._id ? true : false
    );
    ...
    const fetchAssessments = async () => {
      try {
        setIsLoading(true);
        const response = await privateAxios.get(`${ASSESSMENTS}${params.id}`, {
          signal: controller.signal,
        });
        setIsLoading(false);
      ...
      if (section?.layout === "table") {
        const transformedColValues = getTransformedColumns(
          section?.columnValues
            ...
            sectionErrors[question?.uuid] = "This is nummeric field";
            setTabValue(index);
          } else if (
            ...
            sectionErrors[question?.uuid] = "This is alphanumeric field";
            setTabValue(index);
          } else {
  ...
  return (
    <div className="page-wrapper">
      <DialogBox
        ...
        info1={
          <p>
            On Accepting this assessment you need to fill this assessmentin
            given time, and on rejecting this assessment you need to mention
            reason for it
          </p>
        }
        info2={
